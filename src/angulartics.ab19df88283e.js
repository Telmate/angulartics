...
.provider('$analytics', function () {
  ...
  var bufferedCustomTrack = function() {
    cache.custom.push(_.toArray(arguments));
  };
  ...
  var api = {
    ...
    customTrack: bufferedCustomTrack
  };

  var registerCustomTrack = function(fn) {
    api.customTrack = fn;
    angular.forEach(cache.custom, function(eventArgs, index) {
      setTimeout(function() { api.customTrack.apply(api, eventArgs); }, index * settings.eventTrack.bufferFlushDelay);
    });
  };
  ...
  return {
    ...
    registerCustomTrack: registerCustomTrack
  };
})
